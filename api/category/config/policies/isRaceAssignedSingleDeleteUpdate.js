...
module.exports = async (ctx, next) => {
  ...
  // Check which category is trying to be deleted/updated
  const categoryId = ctx.params.id;

  // 1) Get the race id to which the category belongs
  const categoryObject = await strapi.services.category.findOne({ id: categoryId });
  const raceId = categoryObject.race;

  // 2) Check if user has permissions to this race
  const isAllowed = assignedRaces.find((id) => id === raceId);

  // 3) If the user has permissions to the race allow him to delete/update category.
  if (isAllowed) {
    return next();
  }
